                     ...
                     const int a_src, const int a_dest)
{
    ...
    {
        int src = (int)src1.get(i);
        int dest= (int)dest1.get(i);
    ...
    {
        int src = (int)src2.get(i);
        int dest= (int)dest2.get(i);
...
void GraphPlot::plot(const char* a_filename, Graph& a_graph,
                     Array& a_node2objects, Array& a_visitednode, Hash& a_nodes, Array& a_node2queries)
{
    float minx, miny, maxx, maxy;
    boundingbox(a_graph, minx, miny, maxx, maxy);
    ...
    for (HashReader rdr(a_graph.m_nodes); !rdr.isEnd(); rdr.next())
    {
        ...
        for (int i=0; i<node->m_edges.size(); i++)
        {
            Edge* edge = (Edge*)node->m_edges.get(i);
            Node* dest = a_graph.getNode(edge->m_neighbor);
            if (found && a_visitednode.binSearch((void*)edge->m_neighbor) != -1){ int temp=0;}
//                draw.line(node->m_x, node->m_y, dest->m_x, dest->m_y, 0.8f, 0, 0, 0);
...
//    {
//        int path = (int)a_visitednode.get(i);
//        Node* node = a_graph.getNode(path);
//        draw.box(node->m_x-0.01f, node->m_y-0.01f, node->m_x+0.01f, node->m_y+0.01f, 0.8f, 0, 0);
//        delete node;
//    }
