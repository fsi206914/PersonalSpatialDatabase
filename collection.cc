...
{
    return Hash::put((int)a_p,(void*)1);
}

int Set::remove(void* a_p)
{
    return Hash::remove((int)a_p);
}
...
void BinHeap::insert(void* _p)
{
   ...
   while (true)
   {
      ...
      int ret = m_compare != 0 ?
          m_compare(&(*r)->m_p, &_p) : (int)(*r)->m_p - (int)_p;
      if (ret <= 0)
         r = &(*r)->m_right;
      else
         r = &(*r)->m_left;
...
bool BinHeap::exist(void* a_p) const
{
   ...
   while (true)
   {
      ...
      int ret = m_compare != 0 ?
          m_compare(r->m_p, &a_p) : (int)r->m_p - (int)a_p;
