...
Node::Node(const int a_id, float a_x, float a_y, int a_interval=0, watchtowerArray *a_WA = 0):
m_id(a_id), m_x(a_x), m_y(a_y), remainInterval(a_interval)
{
WA = a_WA; personal = false;
}
...
void Node::addEdge(Edge& a_edge)
{
    ...
    m_edges.append(new Edge(a_edge.m_neighbor,a_edge.m_cost,a_edge.m_ID, a_edge.WA, a_edge.m_srcID));
}
...
void Node::fromMem(char* a_mem, int& a_len)
{
    ...
    for (int i=0; i<sz; i++)
    {
        ...
        edge_ID= *(long*)&a_mem[a_len];   a_len += sizeof(edge_ID);
        m_edges.append(new Edge(nid,cost,edge_ID));
    }
}
